   ...
   require_once('connect.php');
   	...
   	if(!empty($_POST['kode_rfid'])){
		...
		$data[] = $alamat;
   		...
   		$sql = "UPDATE `tb_pasien` SET kode_rfid=?,tanggal_masuk=?,nama=?,nik=?,
		tanggal_lahir=?,alamat=?,diagnosa=?,dokter_penanggungjawab=?,kategori_pasien=?,status_pasien=? WHERE id=?";
   		$row = $dbh->prepare($sql);
   		$row->execute($data);
		
   		echo '<script>alert("Berhasil Edit Data");window.location="index.php?halaman=pasien_dewasa"</script>';
	   }
   
   	$id = $_GET['id'];
   	$sql = "SELECT *FROM `tb_pasien` WHERE id= ?";
   	$row = $dbh->prepare($sql);
